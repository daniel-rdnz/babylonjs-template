...
export default class FPSCameraController {
  constructor(
    scene,
    canvas,
    settings = { withCrtEffect: false, withGrain: false, withPixelated: false, withFlashLight: true }
  ) {
    this.scene = scene
    this.canvas = canvas
  ...
  setFlashLight = (camera) => {
    const spotLight = new SpotLight('flashLight', new Vector3(0, 0, 1), new Vector3(0, 0, 1), Math.PI, 20, this.scene)
  ...
  setCamera = (scene, canvas) => {
    const camera = new TargetCamera('PlayerCamera', new Vector3(0, 20, 0), scene)
    camera.target = new Vector3(0, 20, 1)
    camera.inertia = 0
